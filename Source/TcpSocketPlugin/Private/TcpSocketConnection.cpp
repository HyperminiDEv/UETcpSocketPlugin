...
void UTcpSocketConnection::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
	UE_LOG(LogTemp, Log, TEXT("Socket subsystem initialized!"));
	// Podés iniciar la conexión o setear variables acá
}

void UTcpSocketConnection::Deinitialize()
{
	UE_LOG(LogTemp, Log, TEXT("Socket subsystem shutting down!"));
	// Cerrá el socket o liberá recursos acá
	Super::Deinitialize();
}


bool UTcpSocketConnection::Connect(const FString& ipAddress, int32 port, const FTcpSocketDisconnectDelegate& OnDisconnected, const FTcpSocketConnectDelegate& OnConnected,
	const FTcpSocketReceivedMessageDelegate& OnMessageReceived, int32& ConnectionId)
{

	if(SocketLimit > 0 && TcpWorkers.Num() >= SocketLimit)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tcp Socket: Cannot create new connection. Socket limit reached (%d)."), SocketLimit);
		ConnectionId = -1;
		return false;
	}
